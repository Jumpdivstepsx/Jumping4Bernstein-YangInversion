...
int div55(int delta, int16x8_t *fi, int16x8_t *gi, int16x8_t *uvqr)
{
    ...
    int16x8_t f[7], g[7], u[7], v[7], q[7], r[7];
    ...
    for (i = 0; i < 7; i++)
    {
        ...
        uvqr[i + 7] = barrett_fake(v[i]);
        uvqr[i + 14] = barrett_fake(q[i]);
        uvqr[i + 21] = barrett_fake(r[i]);
        uvqr[i + 28] = barrett_fake(f[i]);
        uvqr[i + 35] = barrett_fake(g[i]);
    }
    return (-(int16_t)vgetq_lane_s16(minusdelta_v, 0));
}


int div40(int delta, int16x8_t *fi, int16x8_t *gi, int16x8_t *uvqr, int* res)
{
    int16_t f0, g0;
    int16x8_t f0vec, g0vec, maskvec;
    int16x8_t f[7], g[7], u[7], v[7], q[7], r[7];
