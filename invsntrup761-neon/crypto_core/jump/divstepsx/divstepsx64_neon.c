...
int div64(int delta, int16x8_t *fi, int16x8_t *gi, int16x8_t *uvqr)
{
    ...
    int16x8_t f[8], g[8], u[8], v[8], q[8], r[8];
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        uvqr[i + 8] = barrett_fake(v[i]);
        uvqr[i + 16] = barrett_fake(q[i]);
        uvqr[i + 24] = barrett_fake(r[i]);
        uvqr[i + 32] = barrett_fake(f[i]);
        uvqr[i + 40] = barrett_fake(g[i]);
    }
